...
class RegisterSuperUserView(generics.GenericAPIView):
    ...
    serializer_class = RegisterSuperUser
...
class UserListView(APIView):
    def get_queryset(self):
        ...
        # return User.objects.filter(groups__id=3)
        return User.objects.filter(groups__id=2)

    def get(self, request):
        
        queryset = self.get_queryset()
        # for users in queryset: 
        #     if 3 in users.groups.values_list('id', flat=True):
        #         print("user is : ",users.groups.values_list('id',flat=True))
        #         users.is_varified = True
        #         users.save()
